...
* **Model Optimization**: 최신 객체 인식 모델인 YOLO11n(320x320 Input)을 NCNN 프레임워크로 변환하여 엣지 환경에서의 실시간 추론(Inference) FPS를 극대화했습니다[cite: 11, 17, 20].
* **Finite State Machine (FSM)**: 6개의 클래스(스쿨존 진입/해제, 횡단보도, 3색 신호등) 인식 결과를 바탕으로 자율주행 상태 기계(STOP, SLOW, GO)를 설계 및 운용합니다[cite: 11, 17].
* **In-Vehicle Network**: 판단된 주행 상태(State)는 SocketCAN 인터페이스를 통해 STM32로 0.05초 주기로 실시간 전송됩니다.

### 2. Embedded Control System (STM32F446RE)
* **RTOS Task Scheduling**: FreeRTOS 기반 멀티스레딩 아키텍처를 적용하여 센서 인지(High Priority), 주행 제어(Normal Priority), 요금 UI 갱신(BelowNormal Priority)을 분리하여 자원 점유 최적화 및 실시간성을 확보했습니다[cite: 1].
* **Proportional Control (비례 제어)**: 3채널 초음파 센서의 양측면 데이터를 P-제어 알고리즘으로 연산하여 차동 구동(Differential Drive) 모터의 좌우 PWM Duty Cycle을 동적으로 분배, 팽팽한 트랙 중앙 유지(Center Keeping)를 구현했습니다[cite: 1, 6].
* **Odometry & UI**: 휠 엔코더의 펄스를 외부 인터럽트(EXTI)로 획득하여 물리적 주행 거리(1 Tick = 1.021cm)를 추산하고, 비동기적으로 I2C LCD에 요금을 갱신합니다[cite: 3, 10].
...
* **해결**: Python의 `threading.Lock()`을 활용한 **Producer-Consumer 패턴**을 구현. 백그라운드 스레드에서 프레임을 지속적으로 비워내고, 메인 추론 루프에서는 가장 최신(Latest) 프레임 1장만 읽어가도록 구조를 개편하여 물리적 지연 시간을 ms 단위로 단축했습니다[cite: 11].
...
* **해결**: MCU의 APB1 버스 클럭(90MHz)을 바탕으로 타이머(TIM4) 프리스케일러를 1MHz(1us)로 직접 튜닝(`Prescaler = 90-1`). 내부 카운터 레지스터(`__HAL_TIM_GET_COUNTER`)를 직접 폴링(Polling)하여 정확히 12us의 펄스를 생성하고, Echo 응답에 대한 정밀한 Time-of-Flight 연산을 구현했습니다[cite: 3, 13].
...
* **문제 2**: 3개의 초음파 센서 데이터를 갱신하는 Task와, 이 데이터를 바탕으로 모터를 제어하는 Task 간의 Data Tearing 현상 우려.
    * **해결**: FreeRTOS의 `osMutexAcquire/Release`를 활용한 임계 구역(Critical Section) 보호 래퍼 함수를 구현하여, 전방/좌/우 센서 데이터의 원자성(Atomicity)을 보장했습니다[cite: 1].
